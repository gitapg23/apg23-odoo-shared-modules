...
class CodImmobile(models.Model):
    _inherit = 'immobile.immobile.codice'
    ...
    def create(self, vals_list):
        ...
        records = super(CodImmobile, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        """
        res = super(CodImmobile, self).write(vals)

        if 'owner_company' in vals:
            for record in self:
                analytic_account = record._get_or_create_analytic_account()
                if analytic_account:
                    record.analytic_account_id = analytic_account
    ...
    def _get_or_create_analytic_account(self):
        ...
        """
        self.ensure_one()
        analytic_plan = self.env.ref('huroos_apg23_analitica.analytic_plan_immobili', raise_if_not_found=False)
        if not self.name or not analytic_plan:
            return self.env['account.analytic.account']

        name = str(self.name).upper()
        company_id = self.owner_company.id or self.env.company.id
